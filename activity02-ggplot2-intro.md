...
```
    ...
    ## Installing package into '/home/johnsean/R/x86_64-pc-linux-gnu-library/4.1'
...
**Response**: Gentoo has greater length that the other 2, the other two
are similar
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species, shape = species)) +
  geom_jitter()
```
...
**Response**: this is very similar to the boxplots
...
**Response**: The firt one for both of them (????)
...
**Response**: if i cahnged the alpha so the dots are less dark
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species)) +
  geom_boxplot() + geom_jitter(aes(x = flipper_length_mm, y = species, color=species))
```
...
**Response**: you can see where the density variation of the dots are
for the
...
``` r
ggplot(data = penguins, mapping = aes(x = bill_length_mm, y = sex)) +   geom_boxplot()
```
...
``` r
ggplot(data = penguins, mapping = aes(x = bill_length_mm, y = island)) +   geom_boxplot()
```
...
**Response**: There is not much variability expect between the bill
lenght and the species
...
``` r
ggplot(data = penguins, mapping = aes(x = bill_length_mm, y = body_mass_g, color=species)) +   geom_point()
```
...
**Response**: you have a nice svm separation
...
**Response**: same as the above comment
...
``` r
ggplot(data = penguins, mapping = aes(x = bill_length_mm, y = body_mass_g, color=species)) +   geom_point(alpha=0.5)
```
...
there a few outliers int he middle and a bit of overlap
